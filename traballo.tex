%  PARA TRABALLOS EN GALLEGO USAR (LINEA 12): \usepackage[galician]{babel}
%  PARA TRABALLOS EN CASTELLANO USAR (LINEA 13): \usepackage[spanish]{babel}
%
% Para los acentos usamos codificacion UTF-8 (LINEA 10): \usepackage[utf8]{inputenc} 
% Si se usase la codificacion es_ES.ISO-8859-1 (LINEA 11): \usepackage[latin1]{inputenc}
% La conversion de acentos se hace con: iconv -f UTF-8 -t ISO-8859-1 filename.tex
%
% Como se incluyen figuras eps hay que compilar con: latex traballo , dvipdf traballo
%

\documentclass[12pt,twoside,a4paper]{book}
% pódense engadir todos os packages necesarios
\usepackage[utf8]{inputenc}
%\usepackage[latin1]{inputenc}
%\usepackage[galician]{babel}
\usepackage[spanish]{babel}
\usepackage{graphicx}
\usepackage[dvips]{epsfig}
\usepackage{amssymb}
\usepackage{eurosym}
\usepackage{float}
\usepackage{latexsym}
\usepackage{a4}
\usepackage{listings}
\usepackage{multirow}
\usepackage{hyperref} %[hidelinks] menús no pdf pero non leva ben co package galician
\usepackage{url}
\usepackage{rotating}
\usepackage{caption}
\usepackage[section]{placeins}
\usepackage{booktabs}
\usepackage{tabularx} % Better tables
\usepackage{tabu}
\usepackage{enumitem}
\usepackage{xcolor}
\usepackage{xstring, xifthen}
\usepackage[nottoc, notlot, notlof, notindex]{tocbibind}
\usepackage[numbers, square, super]{natbib}

%Formato de enlaces en PDF
\hypersetup{
  pdfborderstyle={/S/U/W 1}% border style will be underline of width 1pt
}

\input{options.tex}

\begin{document}
%%Se reeditan estas funciones para sacar el texto en gallego usando el idioma spanish, porque galician no funciona con hyperref
\renewcommand{\contentsname}{Índice xeral}
\renewcommand{\listtablename}{Índice de cadros}
\renewcommand{\tablename}{Cadro} 
\include{capitulos/titulo}\cleardoublepage
\include{capitulos/certificacion}\cleardoublepage % paxina de certificación (optativa)
%\include{capitulos/agradecementos}\cleardoublepage % paxina de agradecementos (optativa) 
\include{capitulos/resumo}\cleardoublepage % páxina de resumo (optativa) 

\pagestyle{plain}
\tableofcontents
\listoffigures
\listoftables

% Agora incluimos os capítulos. Cambiamos a numeración e as cabeceiras
\cleardoublepage
\pagenumbering{arabic}
\setcounter{page}{1}
\pagestyle{headings}
\include{capitulos/introducion}\cleardoublepage
\include{capitulos/xestiondoproxecto}\cleardoublepage
\include{capitulos/analise}\cleardoublepage
\include{capitulos/deseno}\cleardoublepage
\include{capitulos/implementacion}\cleardoublepage
\include{capitulos/conclusions}\cleardoublepage

% Aquí empezan os apéndices
\appendix
%\include{capitulos/apendicea}\cleardoublepage
\include{capitulos/apendiceb}\cleardoublepage

%\nocite{*}
\bibliography{bibliografia}
\bibliographystyle{plain}
\end{document}
